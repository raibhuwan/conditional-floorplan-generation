...
# Parse command-line settings for cGAN evaluation with hill-climbing refinement.
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate the Pix2Pix-style cGAN with hill-climbing refinement "
            "on the held-out test set using a consistent combined-component "
...
# Evaluate the retained cGAN generator after hill-climbing refinement.
def main():
    args = parse_args()
    ...
    print(
        "Evaluating cGAN + hill-climbing on held-out test samples: "
        f"{len(test_dataset)}"
    )
    print(
    ...
    # Only the retained generator is required for semantic prediction.
    ...
    for test_position, (inputs, targets) in enumerate(
        ...
        # Convert the raw prediction and reference target to NumPy semantic masks.
        pred_mask = (
            predictions[0]
            .cpu()
            .numpy()
        ...
        # Apply the greedy hill-climbing refinement to the raw cGAN prediction.
        ...
        # Reconstruct target and predicted counts using the same combined-mask rule.
        gt_room_count = (
