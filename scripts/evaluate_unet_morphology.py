...
# Parse command-line settings for U-Net evaluation with morphology refinement.
def parse_args():
...
def extract_instances(
    ...
    # Extract components independently for each retained room-type class.
    for class_id in range(NUM_CLASSES):
        ...
        # Component 0 is background, so inspect foreground components only.
        for component_id in range(1, component_count):
            ...
            # Preserve the geometry and class identity required by the
            # adjacency and compactness calculations.
            instances.append(
                {
                    "class_id": class_id,
                    "area": area,
...
def count_rooms_from_semantic_mask(
    ...
    """
    # Merge all non-background and non-wall classes before component counting.
...
def compactness_of_instance(instance_mask):
    ...
    # Use external contours to estimate the component perimeter.
...
def adjacency_edges(instances):
    ...
    # Dilate each instance once so nearby regions can be tested for contact.
    dilated_masks = [
        cv2.dilate(
...
def load_model(device, checkpoint_path):
    ...
    # Require a U-Net model state to avoid loading an incompatible checkpoint.
    ...
    # Restore the retained parameters and disable training-specific behaviour.
...
# Evaluate the retained U-Net after morphology refinement on the held-out test set.
def main():
    args = parse_args()
    ...
    # Load the processed dataset and the fixed train/validation/test split.
    ...
    # Restrict evaluation to the held-out test indices and preserve their order.
    ...
    # Load the validation-selected U-Net checkpoint used for all test predictions.
    ...
    for test_position, (inputs, targets) in enumerate(
        ...
        # Run raw U-Net inference without gradient calculation.
        with torch.no_grad():
            logits = model(inputs)
        ...
        # Apply the fixed morphology procedure to the raw semantic prediction.
        ...
        # Verify that the encoded condition agrees with the reconstructed target count.
        ...
        # Measure semantic overlap on the refined prediction while excluding background.
        miou = mean_iou(
            pred_refined,
            gt_mask,
            ignore=(BG,),
        ...
        # Class-specific components remain necessary for adjacency and
        # compactness, but they are not used for connected-region count error.
        ...
        # Evaluate the refined mask without clipping it to the support boundary.
        boundary_violation = (
        ...
        # Compare class-level adjacency relationships after refinement.
        ...
        # Calculate compactness for retained ground-truth and predicted components.
    ...
    # Stop rather than writing an empty CSV if evaluation produced no samples.
    if not rows:
        raise RuntimeError(
    ...
    # Save the sample-level metric records to CSV.
    with open(
        args.out_csv,
        "w",
        newline="",
    ...
    # Aggregate sample-level values to produce overall held-out test means.
    mean_miou = float(
        np.mean(
            [
                row["miou_no_bg"]
    ...
    # Count any disagreement between the encoded condition and reconstructed target count.
